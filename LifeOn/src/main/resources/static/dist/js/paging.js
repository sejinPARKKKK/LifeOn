...
function pagingUrl(current_page, total_page, url) {
    ...
    try {
        ...
        if(total_page - currentPageSetup > numPerBlock) {
            result += '<a href="' + url + 'page=' + n + '">></a>';
            result += '<a href="' + url + 'page=' + total_page + '">>></a>';
        }
        result += '</div>';
...
function pagingMethod(current_page, total_page, methodName) {
    ...
    try {
        ...
        n = current_page - numPerBlock;
        if(total_page > numPerBlock && currentPageSetup > 0) {
            result += '<a onclick="' + methodName + '(1);"><</a>';
            result += '<a onclick="' + methodName + '(' + n + ');"><<</a>';
        }
        ...
        if(total_page - currentPageSetup > numPerBlock) {
            result += '<a onclick="' + methodName + '(' + n + ');">></a>';
            result += '<a onclick="' + methodName + '(' + total_page + ');">>></a>';
        }
        result += '</div>';
