			...
			<tr class="border">
				...
				<td style="text-align: left;">
					<div>
								...
								<div class="reply-menu" style="display: none; font-size: 11px; background: #fff; color: #333; position: absolute; border: 1px solid #e0e0e0; border-radius: 4px; margin-top: 15px; cursor: pointer;">
									<c:choose>
										<c:when test="${sessionScope.member.nickName == prize.nickname}">
											<div class="deleteReply reply-menu-item" data-replyNum="${prize.rpnum}" data-pageNo="${pageNo}">삭제</div>
										</c:when>
										<c:when test="${sessionScope.member.grade >= 1}">
											<div class="deleteReply reply-menu-item" data-replyNum="${prize.rpnum}" data-pageNo="${pageNo}">삭제</div>
											<div class="blindReply reply-menu-item" data-replyNum="${prize.rpnum}" data-replyBlind="${prize.rpblind}">신고</div>
										</c:when>
										<c:otherwise>
						...
						<div class="content" style="width: 90%;">${prize.rpcontent}</div>
						<div style="font-size: 12px; color: #777; padding-top: 5px; display: flex; justify-content: space-between; flex-wrap: wrap;">
							...
							<div style="display: flex; justify-content: flex-end; flex-wrap: wrap;">
								<span data-memberLiked="${prize.memberLiked}" style="padding-right: 20px; cursor: pointer;">
									<span class="btnSendReplyLike" data-replyNum="${prize.rpnum}" data-replyLike="1" title="좋아요" style="padding: 3px;">
										<i class="bi ${prize.rplike == 1 && prize.memberLiked == prize.rpnum ? 'bi-heart-fill disLikeColor':'bi-heart'}"></i>
										<span>${prize.likeCount}</span>
									</span>
									<span class="btnSendReplyLike" data-replyNum="${prize.rpnum}" data-replyLike="0" title="싫어요" style="padding: 3px;">
										<i class="bi ${prize.rplike == 0 && prize.memberLiked == prize.rpnum ? 'bi-heartbreak-fill likeColor':'bi-heartbreak'}"></i>
										<span>${prize.disLikeCount}</span>
									</span>	        
								</span>
							</div>
						</div>
