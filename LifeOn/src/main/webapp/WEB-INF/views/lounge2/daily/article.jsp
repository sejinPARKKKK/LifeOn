...
<main class="min-vh-100">
    ...
    <div class="body-title">
    	<em style="font-size: 30px; font-weight: 800; text-shadow: 2px 2px 5px rgba(0, 0, 0, 0.3);">일상 속의 이야기, 나누고 함께해요!</em>
	</div>
				...
				<div class="freeForm">
							...
							<tr>
								<td colspan="2" style="word-wrap: break-word;">
									...
									<c:if test="${not empty prevDto}">
										<a href="${pageContext.request.contextPath}/lounge2/daily/article/${prevDto.psnum}?${query}">이전글 : ${prevDto.subject}</a>
									</c:if>
								</td>
							</tr>
							<tr>
								<td colspan="2" style="word-wrap: break-word;">
									...
									<c:if test="${not empty nextDto}">
										<a href="${pageContext.request.contextPath}/lounge2/daily/article/${nextDto.psnum}?${query}">다음글 : ${nextDto.subject}</a>
									</c:if>
								</td>
							</tr>
						</tbody>
					...
					<table class="table table-borderless">
						<tr>
							<td class="text-start">
								<c:choose>
									<c:when test="${sessionScope.member.nickName == dto.nickname}">
										<button type="button" class="ssbtn" onclick="location.href='${pageContext.request.contextPath}/lounge2/daily/update?psnum=${dto.psnum}&page=${page}';">수정</button>
									</c:when>
								</c:choose>
							...
							<td class="text-end">
								<button type="button" class="ssbtn" onclick="location.href='${pageContext.request.contextPath}/lounge2/daily?${query}';">리스트</button>
							</td>
						</tr>
					</table>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="myDialogModalLabel">이미지보기</h5>
	...
	<script type="text/javascript">
		function deleteOk() {
			if (confirm('게시글을 삭제 하시겠습니까?')) {
				...
				let url = '${pageContext.request.contextPath}/lounge2/daily/delete?' + qs;
				location.href = url;
			}
		}
	</script>
...
$(function() {
    ...
    $('input[name="repr"]').on('change', function() {
 	...
 	$('#submitReport').on('click', function() {
		...
		let url = '${pageContext.request.contextPath}/lounge2/daily/boardBlind';
		let repan = '${dto.psnum}';
...
$(function() {
	$('.btnSendBoardLike').click(function() {
		...
		let url = '${pageContext.request.contextPath}/lounge2/daily/insertBoardLike';
		let psnum = '${dto.psnum}';
...
function listPage(page) {
	let url = '${pageContext.request.contextPath}/lounge2/daily/listReply';
	let rpnum = '${dto.rpnum}';
	let psnum = '${dto.psnum}';
...
$(function(){
	$('.btnSendReply').click(function(){
		...
		let url = '${pageContext.request.contextPath}/lounge2/daily/insertReply';
...
$(function() {
	$('.reply').on('click', '.deleteReply', function() {	
		...
		let url = '${pageContext.request.contextPath}/lounge2/daily/deleteReply';
...
$(function() {
	$('.reply').on('click', '.btnSendReplyLike', function() {	
		...
		let url = '${pageContext.request.contextPath}/lounge2/daily/insertReplyLike';
...
$(function() {
	$('.reply').on('click', '.blindReply', function() {	
		...
		let url = '${pageContext.request.contextPath}/lounge2/daily/replyBlind';
