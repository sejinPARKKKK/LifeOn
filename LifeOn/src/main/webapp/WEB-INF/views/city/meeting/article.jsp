					...
					<table class="table board-article" style="table-layout: fixed;">
						...
						<tbody>
							<tr>
								...
								<td align="right">
									<div>${dto.reg_date} | 조회수 ${dto.hitCount}회</div>
								</td>
							</tr>
							...
							<td class="text-start">
								<c:choose>
									<c:when test="${sessionScope.member.nickName == dto.nickname}">
										<button type="button" class="ssbtn" onclick="location.href='${pageContext.request.contextPath}/city/meeting/update?psnum=${dto.psnum}&page=${page}';">수정</button>
									</c:when>
								</c:choose>
	...
	<script type="text/javascript">
		function deleteOk() {
			if (confirm('게시글을 삭제 하시겠습니까?')) {
				...
				let url = '${pageContext.request.contextPath}/city/meeting/delete?' + qs;
				location.href = url;
			}
		}
	</script>
